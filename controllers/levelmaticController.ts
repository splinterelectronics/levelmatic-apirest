...
export default class LevelmaticController {
  ...
  public async existById(req: any, reply: FastifyReply) {
    try {
      const { idLevelmatic } = <ILevelmaticID>req.body;
      const levelmaticExist = await service.getLevelmaticById(idLevelmatic);
